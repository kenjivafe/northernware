...
export default function ThemeToggle() {
  ...
  return (
    <div className={`
      ...
      /* Mobile: Top Right, Horizontal, smaller */
      top-5 right-14 flex flex-row items-center gap-1 py-1 px-1.5
      md:top-1/2 md:-translate-y-1/2 md:right-6 md:flex-col md:gap-4 md:py-3 md:px-3
      bg-nw-graphite/80 dark:bg-nw-white/80 backdrop-blur-xl border border-nw-graphite/20 rounded-full shadow-2xl
    `}>
      <div className="flex flex-row md:flex-col gap-0.5 md:gap-3">
        {modes.map((mode) => (
          <button
            key={mode.id}
          ...
          >
            <div className={`p-1 md:p-3.5 rounded-full transition-all duration-300 relative ${
              theme === mode.id 
              ...
              <span className="iconify text-[10px] md:text-xl" data-icon={mode.icon}></span>
            </div>
          </button>
        ))}
      </div>
